...
const HeaderSetting = ({ selectTab, handleClickTab, tab }: HeaderSettingProps) => {
  ...
  return (
    <div className="h-18 flex w-full flex-col justify-start text-slate-700 ">
      ...
      <div className="flex w-full items-center justify-start rounded-t-lg border-b border-gray-200">
        {tab.map((item) => (
          <div
            role="button"
            tabIndex={0}
            ...
            className={twMerge(
              'relative px-5 py-1 text-sm font-medium text-slate-600',
              selectTab === item.id && 'rounded-t-lg  border-b-0 border-gray-200 text-slate-900 ',
            )}
          >
            {item.label}
            {item.id === selectTab && (
